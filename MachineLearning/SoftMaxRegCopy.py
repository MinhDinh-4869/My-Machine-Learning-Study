...
def softmaxregression(X, y, eta, epochs):
	...
	for i in range(epochs):
		...
		for j in x_id:
			...
			yi = y[j].reshape(1, C)
			a = np.array([StableSoftMax(xi[0].dot(W_temp.T))])
			#dw = (a1 - y[j]).reshape(1,C).T.dot(X[j].reshape(1,d+1))
			W_temp = W_temp - eta*((a - yi).T.dot(xi))

		
		count+=1
		if count % check_after == 0:
			print(count)
			if np.linalg.norm(W_temp - W[-check_after]) < 1e-4:
				print("loop no. ", count)
				return W
		W.append(W_temp)
	return W

W_out = softmaxregression(X, y, 0.05, 500)
W = W_out[-1]
